...
build_local:
	go mod tidy
	go build -ldflags '${LDFLAGS}' -o demo-devops
build:
	go mod tidy
	docker build $(DOCKER_BUILD_ARGS) -t $(DOCKER_USER)/demo-devops:$(APP_VERSION) -f ./build/Dockerfile .
lint:
ifndef GOLINTER
...
artifact_linux:
	CGO_ENABLED=0 GOOS=linux go build -ldflags '${LDFLAGS}' -o demo-devops-linux

artifact_darwin:
	CGO_ENABLED=0 GOOS=darwin go build -ldflags '${LDFLAGS}' -o demo-devops-darwin
...
publish:
	docker push $(DOCKER_USER)/demo-devops:$(APP_VERSION)
